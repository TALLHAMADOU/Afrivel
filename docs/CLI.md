...
Convention de nommage : **`namespace:action`** (familier artisan/Laravel). Implémentation **Rust** : crate `afrivel-cli` (clap), binaire `afrivel` installé via `cargo install afrivel`.
...
| `--verbose` | Détails + backtrace Rust (debug). |
...
### Bootstrap & projet (hors-ligne, global)
| Commande | Rôle |
|----------|------|
...
| `completion <shell>` | Complétion bash/zsh/fish (via `clap_complete`). |

### Génération (hors-ligne, codegen `minijinja`)
| Commande | Rôle |
|----------|------|
...
### Introspection (hors-ligne, lit le manifeste)
| Commande | Rôle |
|----------|------|
...
2. **Résolution templates** : `.jinja` embarqués dans le binaire (`rust-embed`/`include_str!`), rendus avec `minijinja` (`model.rs.jinja`, `controller.rs.jinja`, `repository.rs.jinja`, `service.rs.jinja`, `request.rs.jinja`, `resource.rs.jinja`, `migration.rs.jinja`, `routes.rs.jinja`, `test.rs.jinja`, `lib.rs.jinja`, `Cargo.toml.jinja`).
...
- Codes de sortie : `0` ok, `1` erreur utilisateur, `2` erreur interne. Backtrace Rust seulement sous `--verbose`.
...
| Mismatch `afrivel_version` (CLI globale ↔ deps du projet) | `warn` clair : CLI plus récente/ancienne que les crates `afrivel-*` du projet. |
...
```
1. Watcher (crate `notify`) : app/src/, modules/*/src/, config/, tests/
2. Debounce ~300ms
